...
//   if (preferences.avatar !== undefined && preferences.avatar >= 0) {
//     const avatarKey = `avatar${preferences.avatar}`;
//     if (backgroundImages[avatarKey]) return backgroundImages[avatarKey];
//   }
//   return backgroundImages.avatar0;
// };

// // ----------------- Firestore User ID -----------------
// const userId = "demoUser"; // replace with auth().currentUser.uid if using Firebase Auth
...
//     try {
//       await firestore().collection('users').doc(userId).set({ messages: msgs }, { merge: true });
//     } catch (error) {
...
//         {
//           id: '1',
//           text: `Hi there! 👋 I'm Alexa your personal AI companion. I'd love to get to know you better! What's your name?`,
//           sender: 'ai',
//           timestamp: new Date(),
//         },
//       ]);
...
//           setBotName(data.preferences.botName || 'Alexa');
//         }
//       }
//       await loadCoinsData();
//     } catch (error) {
...
//     }
//   };
//   const saveCoinsData = async (coins: UserCoins) => {
//     try {
//       await firestore().collection('users').doc(userId).set({ coins }, { merge: true });
//     } catch (error) {
...
//         }
//       }
//       const initialCoins: UserCoins = { coins: 5, messagesUsed: 0, totalMessages: 10 };
//       await saveCoinsData(initialCoins);
//       setUserCoins(initialCoins);
//     } catch (error) {
...
//   // ----------------- Effects -----------------
//   useEffect(() => { loadPreferences(); loadSavedMessages(); }, []);
//   useEffect(() => { if (messages.length > 0) saveMessages(messages); }, [messages]);
//   useEffect(() => {
//     if (userPreferences) setBackgroundImage(getBackgroundImage(userPreferences));
//   }, [userPreferences]);
//   useEffect(() => {
...
//       if (state === 'active') loadPreferences();
//     });
//     return () => sub.remove();
//   }, []);

//   // ----------------- Messaging Logic -----------------
//   const canSendMessage = () => userCoins.messagesUsed < userCoins.totalMessages;
//   const useCoins = () => {
//     const updated = { ...userCoins, messagesUsed: userCoins.messagesUsed + 1 };
//     if (updated.messagesUsed % 2 === 0) updated.coins = updated.coins - 1;
//     setUserCoins(updated);
//     saveCoinsData(updated);
//   };
//   const sendMessage = async () => {
//     if (message.trim() === '') return;
//     if (!canSendMessage()) { setShowAdModal(true); return; }
//     useCoins();
...
//     const typingMsg: Message = { id: typingId, text: '...', sender: 'ai', timestamp: new Date() };
//     setMessages([...withUser, typingMsg]);
//     try {
//       const history = withUser.map(msg => ({ role: msg.sender === 'user' ? 'user' : 'assistant', content: msg.text }));
//       const aiResponse = await getAIResponse(userMsg.text, history);
//       const aiMsg: Message = { id: Date.now().toString(), text: aiResponse, sender: 'ai', timestamp: new Date() };
//       setMessages(cur => cur.filter(m => m.id !== typingId).concat(aiMsg));
//     } catch {
//       const errMsg: Message = { id: Date.now().toString(), text: "Sorry, I couldn't process that.", sender: 'ai', timestamp: new Date() };
//       setMessages(cur => cur.filter(m => m.id !== typingId).concat(errMsg));
//     }
//   };

//   // ----------------- UI Render Helpers -----------------
//   const getAvatarImage = (i: number) => (i >= 0 && i < avatarImages.length ? avatarImages[i] : avatarImages[0]);
...
//       {item.sender === 'ai' && (
//         <View style={styles.avatarContainer}>
//           <Image source={userPreferences ? getAvatarImage(userPreferences.avatar) : avatarImages[0]} style={styles.avatarSmall} />
//         </View>
//       )}
//       <View style={[styles.messageBubble, item.sender === 'user' ? styles.userBubble : styles.aiBubble]}>
//         <Text style={[styles.messageText, item.sender === 'user' ? styles.userMessageText : styles.aiMessageText]}>{item.text}</Text>
//       </View>
//       <Text style={[styles.timestamp, item.sender === 'user' ? styles.userTimestamp : styles.aiTimestamp]}>{formatTime(item.timestamp)}</Text>
//     </View>
//   );
...
//   const mainContent = (
//     <SafeAreaView style={styles.container}>
//       <View style={styles.bannerContainer}><MyBannerAd /></View>
//       {/* Header */}
//       <LinearGradient colors={['#00BFFF', '#FF69B4']} start={{ x: 0, y: 0 }} end={{ x: 1, y: 0 }} style={styles.header}>
//         <TouchableOpacity style={styles.headerContent} onPress={toggleDrawer}>
//           <Image source={userPreferences ? getAvatarImage(userPreferences.avatar) : avatarImages[0]} style={styles.avatar} />
//           <View style={styles.headerTextContainer}>
//             <Text style={styles.headerName}>{botName}</Text>
//             <Text style={styles.headerStatus}>Active now</Text>
//           </View>
...
//             <Icon name="monetization-on" size={18} color="#FFD700" />
//             <Text style={styles.coinsText}>{userCoins.coins}</Text>
//             <Text style={styles.messagesLeftText}>{userCoins.totalMessages - userCoins.messagesUsed} left</Text>
//           </View>
//           <TouchableOpacity style={styles.optionsButton} onPress={toggleMenu}>
//             <Icon name="more-vert" size={24} color="#fff" />
//           </TouchableOpacity>
...
//       {/* Messages */}
//       <View style={styles.contentContainer}>
//         <FlatList ref={flatListRef} data={messages} renderItem={renderItem} keyExtractor={i => i.id} contentContainerStyle={styles.messagesList} />
//       </View>
...
//             <TextInput
//               style={styles.input}
//               placeholder={canSendMessage() ? "Type your message..." : "Press Mic to earn coins"}
//               value={message}
//               onChangeText={setMessage}
//               multiline
//               editable={canSendMessage()}
//             />
//             {message.trim() === '' ? (
//               <TouchableOpacity style={styles.micButton} onPress={() => setShowAdModal(true)}>
//                 <LinearGradient colors={['#FF66CC', '#6666FF']} style={styles.micButtonGradient}><Icon name="mic" size={24} color="#fff" /></LinearGradient>
//               </TouchableOpacity>
//             ) : (
//               <TouchableOpacity style={styles.sendButton} onPress={sendMessage}>
//                 <LinearGradient colors={['#FF66CC', '#6666FF']} style={styles.sendButtonGradient}><Icon name="send" size={24} color="#fff" /></LinearGradient>
//               </TouchableOpacity>
//             )}
//           </View>
//         </KeyboardAvoidingView>
...
//   return (
//     <>
//       {backgroundImage ? <ImageBackground source={backgroundImage} style={styles.backgroundImage}>{mainContent}</ImageBackground> : mainContent}
//       <CustomDrawer isOpen={isDrawerOpen} onClose={() => setIsDrawerOpen(false)} navigation={navigation} userName="Alex Johnson" userEmail="alex@example.com" profileImage={userPreferences ? getAvatarImage(userPreferences.avatar) : avatarImages[0]} />
//     </>
//   );
// };
...
//   bannerContainer: { backgroundColor: '#a37d7dff' },
//   header: { paddingVertical: spacing.md, paddingHorizontal: spacing.xl, flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' },
//   headerContent: { flexDirection: 'row', alignItems: 'center' },
//   headerRightContainer: { flexDirection: 'row', alignItems: 'center' },
//   coinsContainer: { flexDirection: 'row', alignItems: 'center', backgroundColor: 'rgba(255,255,255,0.2)', paddingHorizontal: 8, paddingVertical: 4, borderRadius: 15, marginRight: 10 },
//   coinsText: { color: '#fff', fontWeight: 'bold', fontSize: 16, marginLeft: 4 },
//   messagesLeftText: { color: '#fff', fontSize: 12, marginLeft: 6 },
...
//   headerStatus: { fontSize: 14, color: '#fff' },
...
//   aiMessageContainer: { alignSelf: 'flex-start', flexDirection: 'row' },
//   avatarContainer: { marginRight: 8, alignSelf: 'flex-end' },
//   messageBubble: { borderRadius: 18, padding: spacing.md },
//   userBubble: { backgroundColor: '#FF66CC', borderTopRightRadius: 4 },
//   aiBubble: { backgroundColor: '#fff', borderTopLeftRadius: 4 },
//   messageText: { fontSize: 16 },
//   userMessageText: { color: '#fff' },
//   aiMessageText: { color: '#000' },
//   timestamp: { fontSize: 10, marginTop: 4, color: '#888' },
//   userTimestamp: { alignSelf: 'flex-end' },
//   aiTimestamp: { marginLeft: 38 },
...
//   input: { flex: 1, backgroundColor: '#f5f5f5', borderRadius: 20, paddingHorizontal: spacing.md, paddingVertical: spacing.sm, marginHorizontal: spacing.xs, maxHeight: 100 },
//   micButton: { width: 40, height: 40, borderRadius: 20, overflow: 'hidden' },
//   micButtonGradient: { width: '100%', height: '100%', justifyContent: 'center', alignItems: 'center' },
