...
export const SignInScreen = ({ navigation }: SignInScreenProps) => {
  ...
  const handleSuccessfulLogin = async (userToken?: string) => {
    try {
      // Store user token to mark as logged in
      const token = userToken || 'user_logged_in_token'; // Use actual token if available
  ...
  const handleGoogleSignIn = async () => {
    try {
      ...
      await handleSuccessfulLogin(userInfo.idToken || 'google_user_token');
